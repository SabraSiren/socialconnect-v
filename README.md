...
│   │   ├── OptionMenu/     # Выпадающее меню в профиле (Delete avatar, Logout, курс валют)
...
│   ├── App.module.scss     # Стили App (БЭМ, SCSS)
│   ├── main.tsx            # Точка входа (Vite)
│   └── globals.scss        # Глобальные переменные темы
...
- `PATCH /auth/me/avatar` — смена аватара
- `POST /auth/refresh` — обновление JWT (вызывается автоматически при 401)

### Файлы и изображения

- `POST /files` — загрузка файла (multipart/form-data). Используется для картинок в постах и аватара. Поддерживаются форматы JPG, JPEG, PNG; ограничение размера — 512 KB.
- Изображения отдаются по URL вида `GET /files/<file_id>` (используется в `src` у `<img>`). 
Есть возможность установки/смены (по клику на аватар) и удаления (в меню) аватара, добавления картинки в пост, удаление из превью постформы.
...
В проекте используются **БЭМ** (Block Element Modifier) для именования классов и **SCSS** для написания стилей.

- Стили компонентов лежат в файлах **`.module.scss`** (например, `App.module.scss`, `Post.module.scss`).
- Глобальные переменные (цвета, отступы) заданы в **`src/globals.scss`** в `:root` и доступны через `var(--имя)`.
- В модулях используется **вложенность** по БЭМ: блок с вложенными `&__element` и `&--modifier`, псевдоклассы (`&:hover`, `&:focus`), медиа-запросы внутри блока.
